...
import { adminRefundCredits } from "@/lib/credits/admin-adjustment";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await adminRefundCredits({
    ...
    return NextResponse.json({ error: "Unable to refund credits." }, { status: 500 });
  }
}
